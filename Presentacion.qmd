...
```{r, echo=FALSE, message=FALSE, fig.cap="Porcentaje de hongos según el tipo ", fig.height=3, fig.width=5}
# Análisis descriptivo
theme_set(theme_bw())
...
Ninguna interaccion es significativa por lo que no se incluyen en el modelo.


Una vez elegidas las variables, se prueba si se debe añadir el diametro del sombrero del hongo de forma lineal o como un factor. Se divide la variable en cuatro grupos de tamaño parecido y se crean dos variables con ellos, una ordinal y la otra categorica. Se introducen al modelo ambas y se comparan las deviance de los modelos. No se rechaza(**p-value:0.20**) la hipotesis nula de no linealidad, por lo que se mantiene en el modelo la variable en su forma original. 
...
Ademas, para verificar la bondad del ajuste del modelo se utiliza el test de Hosmer-Lemeshow con 10 grupos, resultando en un no rechazo(**p-value:0.93**) de la hipotesis nula de bondad del ajuste.
...
Para dar una idea de como cambian las probabilidades, se calculan las razones de odds para cambios de las categorias de las variables.

Al aumentar en 5cm el diametro del sombrero del hongo, la chance de que el hongo sea comestible aumenta en un `r (RO_diameter-1)*100`%.


La chance de que los hongos con sombreros chatos sea comestible es un `r (RO_chato-1)*100`% mas grande que esa misma chance para los hongos con forma redondeada del sombrero.
