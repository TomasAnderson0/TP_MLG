...
Se estima el modelo y se llega al siguiente predictor lineal:
...
```{r, message=FALSE, echo=FALSE, warning=FALSE, fig.height=3, fig.width=5}
...
```
El punto de corte que maximiza la sensibilidad, la especificidad y la precisión global es el 0.459 y el área bajo la curva es de 0.719.
...
Se tiene una precisión global de `r asd[[3]][1]`, una especificidad de `r round(asd[[4]][2],3)` y una sensibilidad de `r round(asd[[4]][1],3)`.
