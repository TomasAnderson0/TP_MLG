...
Se estima que existen entre 2 a 4 millones de hongos en el mundo de los cuales la ciencia a logrado describir solo una pequeña porción de estos. Los hongos de mayor tamaño a veces pueden ser venenosos y otros pueden llegar a ser comestibles. Por ello, es esencial poder distinguir entre estos para evitar posibles inconvenientes que pueden ir desde malestar hasta la muerte.

La estadística nos puede ayudar a estimar cuáles pueden llegar a ser las principales diferencias entre las características observables de los hongos comestibles y venenosos además de la temporada en la que suele crecer. Para ello se cuenta con un conjunto de datos de 250 hongos clasificados en comestibles o venenosas con 8 variables. Estas son:
...
Conseguir un modelo lineal generalizado de regresion logistica para predecir si los hongos son comestibles o no.
...
Primero, se muestra la distribución de si el hongo es comestible.
...
plot1 = muestra %>% group_by(class) %>% count() %>% ggplot() + 
  ...
  scale_fill_manual(values = c("#ba28f0", "#dcc382"), guide = "none") +
...
ggplot() +
  ...
  labs(title = "Diametro del sombrero") +
...
```
La distribución de la altura y el ancho del tallo,y el diametro del sombrero parecen ser parecidas para ambos tipos de hongos. Hay algunas observaciones atipicas en dichas variables que podrian afectar negativamente al momento de plantear un modelo. La temporada con menor porcentaje de hongos comestibles es el verano, con un `r round(100*abs(sum(muestra$class[muestra$season == "Verano"]))/length(muestra$class[muestra$season == "Verano"]),2)`%.

```{r, echo=FALSE, message=FALSE, fig.cap="Variables explicativas vs Tipo de hongo"}
grid.arrange(plot5, plot6, plot7, plot8)
```
 
Para la forma del sombrero, la categoria que tiene el mayor porcentaje de hongos comestibles es la chata con el `r round(100*abs(sum(muestra$class[muestra$cap_shape == "Chato"]))/length(muestra$class[muestra$cap_shape == "Chato"]),2)`% y la menor es otros con el `r round(100*abs(sum(muestra$class[muestra$cap_shape=="Otros"]))/length(muestra$class[muestra$cap_shape == "Otros"]),2)`%.

Para el tipo de lamina, la categoria que tiene el mayor porcentaje de hongos comestibles es la libre con el `r round(100*abs(sum(muestra$class[muestra$gill_attachment == "Libre"]))/length(muestra$class[muestra$gill_attachment == "Libre"]),2)`% y la menor es ninguno con el `r round(100*abs(sum(muestra$class[muestra$gill_attachment=="Ninguno"]))/length(muestra$class[muestra$gill_attachment == "Ninguno"]),2)`%.

Para el color de lamina, la categoria que tiene el mayor porcentaje de hongos comestibles es claros con el `r round(100*abs(sum(muestra$class[muestra$gill_color == "Claros"]))/length(muestra$class[muestra$gill_color == "Claros"]),2)`% y la menor es ninguno con el `r round(100*abs(sum(muestra$class[muestra$gill_color=="Ninguno"]))/length(muestra$class[muestra$gill_color == "Ninguno"]),2)`%.


Para el color del tallo, la categoria que tiene el mayor porcentaje de hongos comestibles es claros con el `r round(100*abs(sum(muestra$class[muestra$stem_color == "Claros"]))/length(muestra$class[muestra$stem_color == "Claros"]),2)`% y la menor es ninguno con el `r round(100*abs(sum(muestra$class[muestra$stem_color=="Ninguno"]))/length(muestra$class[muestra$stem_color == "Ninguno"]),2)`%.
...
Se encontraron significativas a el diametro del sombrero, la forma de sombrero, el color de tallo y la temporada, por lo que se plantea un modelo con todas incluidas y se quitan todas las que no aporten significativamente a la explicacion.
...
Se elimana la temporada del modelo y se prueba incluir las demas variables explicativas que no se incluyeron al principio.
...
Para ver una medida de la capacidad predictiva del modelo se grafica una curva ROC y se observan las predicciones con el punto de corte optimo.
